...
static uint16_t spi_tx_block[_SPI_TX_BLOCK_MAX_]; // default DMA TX buffer
    ...
    uint16_t color[]/* already byte-swapped (LE) */, ili9341_bool_t wait)
{
  ...
  spi_write_blocking(lcd->spi, (const uint8_t*)color, size);
    ...
    int16_t x, int16_t y)
{
  ...
  spi_write_blocking(lcd->spi, (uint8_t *)&color_le, 2U);
    ...
    int16_t x, int16_t y, uint16_t w, uint16_t h)
{
  ...
  if (block_wc > _SPI_TX_BLOCK_MAX_)
    { block_wc = _SPI_TX_BLOCK_MAX_; }
